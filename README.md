...
- `src/notebooks/SubjApplied.ipynb` — апогей триптиха (SubjectiveModeling → PytevIso → приложения): одно дело («пять подозреваемых») прогоняется через три мира-квантали (Гёдель / Гоген / Лукасевич) — один полиморфный код, три вердикта; девиз — Эрлангенская программа (теория = геометрия группы `Aut(L)`); вишенка — мост Лукасевич ≈ вероятность (границы Фреше, состояния MV по Крупа–Панти).
