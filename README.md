...
- **VI. Топосы и неопределённость** — Toposes, Uncertainty, SubjectiveModeling, SubjApplied

`SubjApplied.ipynb` — прикладной финал ствола: апогей триптиха (SubjectiveModeling → PytevIso → приложения), одно дело («пять подозреваемых») через три мира-квантали (Гёдель / Гоген / Лукасевич) — один полиморфный код, три вердикта; девиз — Эрлангенская программа (теория = геометрия группы `Aut(L)`); вишенка — мост Лукасевич ≈ вероятность (границы Фреше, состояния MV по Крупа–Панти).
