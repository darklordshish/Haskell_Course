...
>
> **Фаза B (ветка diagram-arrows, 2026-06-12):** проставлена семантика стрелок.
> Мономорфизмы-хвостики `↣` в pullback-квадратах топосов (omega_pullback, test_omega,
> topos_axioms, topos_conj_neg, topos_impl_disj — стрелки из терминала/подобъектов).
> Естественные преобразования η/ε как двойные стрелки `⇒` в adj_adjunction.
> Изоморфизмы `↔` + `~` в yo_yoneda, yoneda_representable. Эпиморфизмов в наборе нет;
> текстовые `≅`-формулы и уникальные морфизмы (катаморфизм) не трогались. iso-глиф —
> `↔`/`≅` (не `⤖`): однозначно, без смешения с биморфизмом в несбалансированных категориях.
...
## ✅ Фаза 18 — Справочник двойственности Duality.ipynb (ветка duality-notebook, 2026-06-18)

Ноутбук-справочник `src/notebooks/Duality.ipynb` — «парад двойственных конструкций»
вне ствола 0–VI (пополняемый). Единая ось — `C^op`; подача «примитивы → движок
расширений Кана → всё через Кан». Каждый раздел: идея → математика с универсальным
свойством → парная SVG-диаграмма (обращение стрелок) → Haskell обоих членов → «где в курсе».
Для пределов/копределов — два уровня: фиг.1 категорная (форма `J` / расширение вдоль `J→1`),
фиг.2 объектная с явным универсальным свойством (тестовый `x` + единственный медиатор пунктиром).

Инструменты (`src/scripts/duality/`): `build_duality_scaffold.js` — каркас+таблица;
`fill_section.js <slug>` — вписать раздел из `<slug>_md.txt`+`<slug>_hs.txt` (с проверкой
ASCII-only внутри SVG). Прогон — контейнер gibiansky/ihaskell, `nbconvert --execute`.

| Раздел (slug) | Двойственная пара | Haskell |
|---------------|-------------------|---------|
| monoepi | мономорфизм / эпиморфизм | лево-/право-сократимость |
| variance | ко-/контравариантный функтор | Functor / Contravariant |
| algcoalg | F-алгебра / F-коалгебра | cata / ana, Fix |
| kan | Lan / Ran (движок) | Lan/Ran, Codensity |
| limcolim | предел / копредел | конус через Кан вдоль `J→1` |
| termini | терминальный / начальный | () / Void |
| prodcoprod | произведение / копроизведение | (,) / Either |
| equ | уравнитель / коуравнитель | filter / факторизация |
| pbpo | pullback / pushout | list-comp / факторизация |
| adj | сопряжения через Кан | curry / uncurry |
| monadcomonad | монада / комонада | Codensity = Ran f f, Density = Lan f f |

Все 11 разделов — 0 ошибок в контейнере. Критерии к диаграммам зафиксированы в памяти.

---

## ✅ Фаза 19 — Этюд SetOp.ipynb: категория Set^op (ветка setop-notebook, 2026-06-23)

Ноутбук-этюд `src/notebooks/SetOp.ipynb` — разглядывание противоположной категории
множеств `Set^op` с трёх сторон, всё в сравнении с `Set`. Вне ствола 0–VI, рядом с
`Duality.ipynb`. Спека: `docs/superpowers/specs/2026-06-18-setop-notebook-design.md`;
план: `docs/superpowers/plans/2026-06-18-setop-notebook.md`.

Инструменты (`src/scripts/setop/`): `build_setop_scaffold.js` (каркас) и `fill_section.js`
(вписывает `<slug>_md.txt`+`<slug>_hs.txt`, выносит инлайновые `<svg>` во внешние файлы
`src/diagrams/setop/*.svg` — как в Duality). 16 ячеек, 9 SVG, 4 исполнимых этюда.

| Раздел | О чём |
|--------|-------|
| intro | определение `Set^op`: стрелка `A→B` = функция `B→A` |
| limits | (ко)пределы и mono/epi: терминал = `∅`, `×↔⊔`, инъекция↔сюръекция |
| notccc | не декартово замкнута (нет ко-экспонент), не топос |
| caba | `Set^op ≃ CABA`: powerset контравариантен, предикат-трансформер (`Op r a`) |
| partitions | субобъекты = разбиения; `Π(3)=M₃` недистрибутивна (логики нет) |
| coheyting | ко-Гейтинг, граница `∂a≠⊥` (паранепротиворечивость), мост `¬¬` |
| dualities | пространства против алгебр: Стоун, Gelfand, Spec, локали |
| cps | Haskell-мост: `Op`/`Cont` (двойное отрицание), применения, подводные камни |

Все разделы — 0 ошибок в контейнере. Грабли IHaskell (`{-` в комментариях, многострочные
определения, top-level `;` глушат вывод ячейки) зафиксированы в памяти.

---

## 🔧 Фаза 20 — Аудит и чистка (ветка audit-cleanup, начато 2026-06-23)

> Полный прогон-аудит всех 26 ноутбуков (сканер `node` + ручная проверка).
> Подробности и таблицы — `src/AUDIT.md`. Работаем по порядку убывания критичности.

### 20.1 🔴 Содержательные ошибки
- [x] **ComonadTransformers.ipynb** — ячейка `navDemo` не компилировалась: `StateT s (Store s)`
  концептуально неверно (StateT требует базу-`Monad`, а `StoreT` — комонада). Переписаны
  и код, и markdown §8: слои `State`+`Store` держим рядом через сопряжение `State s -| Store s`.
  Перепрогон в контейнере — **0 ошибок**. *(2026-06-23)*
- [x] **DesignShowcase.ipynb** — **ложное срабатывание**: `filename.svg` стоит внутри
  код-блока (иллюстрация шаблона), не рендерится; отсутствие setup — by-design (дизайн-дока).
  Правок не требует.

### 20.2 🟠 Сквозные огрехи
- [x] **hlint-шум в сохранённых выводах** — **решено оставить как есть**: подсказки
  линтера полезны как часть обучающего опыта (интересно видеть, что он предлагает).
  Механизм глушения зафиксирован на будущее: `:opt no-lint` в setup-ячейке. *(2026-06-23)*
- [x] **Пустая колонка «Суть» в TOC** — после проверки реально пусты 5 (не 8):
  YonedaLemma, FunctorHierarchy, Concurrency, DistributedHaskell, GPUHaskell — **заполнены**.
  Adjunctions, Monads — ложные срабатывания (уже заполнены); README — третья колонка
  «Уровень», пустые ячейки = строки-разделители модулей (by-design). *(2026-06-23)*

### 20.3 🟡 Точечное
- [x] **YonedaLemma.ipynb** — транслитерация унифицирована: «Енед»→«Ёнед» (5),
  «Лайнстер»→«Лейнстер» (1). README/карта «Енед» не содержат. *(2026-06-23)*

### 20.4 ⚪ Проверить (возможно by-design)
- [x] Profunctors.ipynb — оглавление **было** (`## Contents`), но по-английски —
  переведено на русский по шаблону «📌 Содержание / # / Тема / Суть». *(2026-06-23)*
- [x] Uncertainty.ipynb — TOC-таблицы не было; **добавлена** «📌 Содержание» (9 секций).
  *(Отдельно замечено: H1 на английском + секции 1–8 на уровне `#` вместо `##` — не трогал.)*
- [x] Duality / SetOp — нет навигации **by-design**: справочники/этюды вне ствола 0–VI.

### 20.5 Инфраструктура
- [x] `src/scripts/` — 31 одноразовый скрипт перенесён в `_archive/` (`git mv`); на верхнем
  уровне остались универсальные инструменты. *(2026-06-23)*
- [x] `src/scripts/check_status.py` — список ноутбуков теперь из `coursemap/course_order.js`;
  пути — относительно скрипта. *(2026-06-23)*
- [x] `src/lib/.gitignore` — добавлены `*.dyn_hi`, `*.dyn_o` (требует commit/push в репо
  сабмодуля SubjectiveModeling). *(2026-06-23)*
...
### Субъективное моделирование: варианты шкал — выполнено (2026-07-09)
- ✅ Библиотека (сабмодуль SubjectiveModeling): квантали `Goguen`/`Luka`/`Trop`, класс
  `InvolutiveQuantale`, деквантование Маслова, полиморфный слой `SubjModelQ` + проекторы
  варианта-2; тесты 34 → 46, 0 провалов
- ✅ `PytevIso.ipynb` §9 «кузница шкал» — варианты Пытьева как квантали, `Aut(L)`
  (исправлено: решётка-ромб, не цепь — `Γ_S` и `Aut(Goguen)` несравнимы)
- ✅ `SubjApplied.ipynb` (16 ячеек, 2 SVG) — апогей триптиха: дело о пяти подозреваемых
  через три мира; Эрлангенская программа; мост Лукасевич ≈ вероятность
- ✅ Регистрация в README.md / README.ipynb; вывод ячеек по-русски; 0 ошибок в контейнере
- ✅ Повышен до ствола (VI.4): добавлен в `course_order.js`, авто-NAV (SubjectiveModeling ⇄
  SubjApplied), на карте — конечная синей ветки. Duality/SetOp/PytevIso — тупики-справочники.
